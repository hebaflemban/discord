...
const Sidebar = ({ user, addChannel, logout }) => {
    ...
    return (
        <ul className={toggleSideBar ? "navbar-nav bg-gradient-piggypink sidebar sidebar-hidden sidebar-dark accordion" : "navbar-nav bg-gradient-piggypink sidebar accordion"} id="accordionSidebar">
            {!toggleSideBar ?
                <>
                    <a className="sidebar-brand d-flex align-items-center justify-content-center mt-5" href="index.html">
                    ...
                    <li class="nav-item mx-3 text-center">
                        <button onClick={toggleView} class="btn btn-light btn-circle mt-0 mb-2">
                            <span className="h3">+</span></button>
                        <div className={isHidden ? "d-none" : "input-group mb-3"}>
                            <input type="text" onChange={(e) => onChange(e.target.value)} className="form-control" placeholder="new channel name" value={chnlName} />
                            <div className="input-group-append">
                                <button value="submit" onClick={() => handleClick()} class="input-group-text" >
                                    <span>Create</span>
                                </button>
                            </div>
                        </div>
            ...
            <li class="nav-item mx-3 my-4 text-center" >
                <button class="btn btn-circle btn-outline-light btn-lg" onClick={hideSideBar}>
                    {toggleSideBar ?
